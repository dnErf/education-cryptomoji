...
const swapMoji = (ownerButes, toRemove, toAdd) => {
  const owner = decode(ownerButes);
...
const acceptResponse = (context, publicKey, {offer, response}) => {
  ...
  return context.getState([ offer ])
    .then(newState => {
      ...
      const decodeOffer = decode(state[offer]);
      const decodedResponse = decodeOffer.responses[response];
      if (!decodedResponse) {
