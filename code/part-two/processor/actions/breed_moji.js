...
const combineDna = (sire, breeder, prng) => {
  ...
  return sireGenes
    ...
    .map(gene => ('0'.repeact(HEX_SIZE) + gene.toString(16)).slice(-HEX_SIZE))
    .join('');
};

// combines the dna of a sire and breeder, creating a new moji for 
// the breeder's owner
      ...
      return context.getState([ listing ])
        .then(state => {
          ...
          const child = {
            ...
            owner: publicKey,
            sired: [],
            bred: [] // is this supposed to be breed ?
          };
