...
const isValidTransaction = transaction => {
  ...
  const 
    toSign = transaction.source + transaction.recipient + transaction.amount;
...
const isValidBlock = block => {
  // Your code here
  const transactionString = block.transaction.map(t => t.signature).join('');
...
const breakChain = blockchain => {
  // Your code here
  blockchain.blocks[1].transactions[0].amount = 9999999999;
};
